...
export default function LapChart({ results, drivers, selectedCode, totalLaps, currentLap }) {
  ...
  // Pseudorandom but stable position-by-lap (deterministic from code+lap)
  function hash(s, n) {
  ...
  const lines = results.slice(0, POS).map(r => {
    ...
    for (let lap = 1; lap <= N; lap++) {
      ...
      // Force final pos to converge to actual classification
      const conv = (lap / N);
      const target = r.pos;
      const blended = Math.round(pos * (1 - conv) + target * conv);
